...
- **The arch rules as first drafted.** `legacy stays contained` expected
  `LSNepomuceno\Signet\Sign` to be deprecated, but that namespace was
...
| 5b | ✅ Remove deprecated API, part 1 | **done**: `Entities\*` and the legacy constants dropped, `Data\*` final, idiomatic enum backing values, arch rule guarding the removal, [the Laravel package's UPGRADE.md](https://github.com/lsnepomuceno/laravel-a1-pdf-sign/blob/main/UPGRADE.md) | n/a |
