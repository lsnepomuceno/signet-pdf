...
it('reads back the level it wrote', function (CertificationLevel $level) {
    expect(resolve(CertificationReader::class)->level(certified($level)))->toBe($level);
})->with([
...
it('reports the certification through the validator', function () {
    $report = resolve(SignatureValidator::class)->validate(certified(CertificationLevel::FormFilling));
...
it('certifies at form-filling by default, since no-changes would refuse the next signer', function () {
    expect(resolve(CertificationReader::class)->level(certified()))->toBe(CertificationLevel::FormFilling);
});

it('accepts the level as a configuration string', function () {
    expect(resolve(CertificationReader::class)->level(certified('annotations')))
...
it('reports no certification when the document carries none', function () {
    expect(resolve(CertificationReader::class)->level(Files::read(resource('test.pdf'))))->toBeNull();
});
...
it('ignores a /Perms that names a signature carrying no transform', function () {
    ...
    expect(resolve(CertificationReader::class)->level($pdf))->toBeNull();
});
