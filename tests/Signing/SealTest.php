...
it('renders a seal carrying the certificate identity', function () {
    ...
    $seal = resolve(SealRenderer::class)->render(testCertificate());
...
it('renders an opaque JPEG when transparency is turned off', function () {
    setConfig('seal.transparent', false);
...
it('honours the configured font colour and size', function () {
    setConfig('seal.font.size', 'small');

    $small = resolve(SealRenderer::class)->render(testCertificate());
    $large = resolve(SealRenderer::class)->render(testCertificate(), FontSize::Large);
...
it('gives each signature its own seal, independent of the ones before it', function () {
    ...
    $signedPath = $second->save(tempFile('.pdf'));
    $report = signet()->validate($signedPath);
...
it('draws the lines the layout names, instead of the certificate', function () {
    ...
    // (docs/decisions/0023-a-seal-that-can-be-transparent.md).
    $default = resolve(SealRenderer::class)->render(testCertificate());

    $custom = resolve(SealRenderer::class)->render(
        testCertificate(),
        layout: LSNepomuceno\Signet\Data\SealLayout::saying([
...
it('does not draw a line the layout gives no baseline for', function () {
    ...
    $twoRows = resolve(SealRenderer::class)->render(
        testCertificate(),
        layout: new LSNepomuceno\Signet\Data\SealLayout(
            lines: ['first', 'second', 'third'],
            rows: [80, 150],
        ),
    );

    $twoLines = resolve(SealRenderer::class)->render(
        testCertificate(),
        layout: new LSNepomuceno\Signet\Data\SealLayout(
            lines: ['first', 'second'],
            rows: [80, 150],
...
it('moves the text where the layout puts it', function () {
    $left = resolve(SealRenderer::class)->render(
        testCertificate(),
        layout: new LSNepomuceno\Signet\Data\SealLayout(lines: ['x'], rows: [80], x: 20),
    );

    $right = resolve(SealRenderer::class)->render(
        testCertificate(),
        layout: new LSNepomuceno\Signet\Data\SealLayout(lines: ['x'], rows: [80], x: 400),
    );
...
it('raises when the seal image is not there', function () {
    expect(fn() => resolve(SealRenderer::class)->fromImage('/no/such/seal.png'))
        ->toThrow(LSNepomuceno\Signet\Exceptions\FileNotFoundException::class);
});
