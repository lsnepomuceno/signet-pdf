...
it('lets every failure be caught as one', function (string $class) {
    expect(class_exists($class) || interface_exists($class))->toBeTrue()
        ->and(is_a($class, SignetException::class, allow_string: true))->toBeTrue();
})->with(function () {
    $classes = [];
...
it('names a wrong password as a wrong password', function () {
    ...
    expect(fn() => resolve(CertificateReader::class)->read((string) file_get_contents($pfxPath), 'not the password'))
...
it('keeps a wrong password catchable as what it used to be', function () {
    ...
    expect(fn() => resolve(CertificateReader::class)->read((string) file_get_contents($pfxPath), 'wrong'))
...
it('does not call a corrupt bundle a wrong password', function () {
    ...
    try {
        resolve(CertificateReader::class)->read('this is not a PKCS#12 bundle at all', 'anything');
