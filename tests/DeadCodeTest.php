...
function deadCodeUnusedInScope(array $tokens, int $open, int $close): array
{
    ...
    for ($index = $open + 1; $index < $close; $index++) {
        ...
        // A static property is not a local variable, and `self::$x = null` is
        // not a dead assignment: the write is the whole point and the read
        // happens in another method entirely. `$this->x` never reaches here
        // because only `$this` tokenises as a variable in that form, but
        // `self::$x` puts the property name in a T_VARIABLE, so it needs
        // saying. Reported against `Harness::reset()` the day it was written.
        if (deadCodePreviousMeaningful($tokens, $index) === T_DOUBLE_COLON) {
            continue;
        }
...
/**
 * The token before this one, ignoring trivia, as its type.
 *
 * The type rather than the text, because the one caller asks about
 * T_DOUBLE_COLON and comparing `'::'` would also match a string literal
 * containing it.
 *
 * @param  array<int, array{0: int, 1: string, 2: int}|string>  $tokens
 */
function deadCodePreviousMeaningful(array $tokens, int $index): ?int
{
