 ...
 * the thing deliberately absent. Checking only `LSNepomuceno\Signet`
...
function specSymbolReferences(string $contents, string $from): array
{
    ...
    // whatever follows a `::`.
    preg_match_all(
        '/\\\\?(LSNepomuceno\\\\Signet(?:\\\\[A-Za-z_][A-Za-z0-9_]*)+)(?:::([A-Za-z_][A-Za-z0-9_]*))?/',
        $contents,
        $matches,
        PREG_SET_ORDER,
    );
...
it('finds a symbol reference that has stopped resolving', function () {
    ...
    expect(specSymbolResolves('LSNepomuceno\Signet\Signing\IncrementalSigner'))->toBeTrue()
        ->and(specSymbolResolves('LSNepomuceno\Signet\Signing\IncrementalSigner::sign'))->toBeTrue()
        ->and(specSymbolResolves('LSNepomuceno\Signet\Signing\RenamedAwayLongAgo'))->toBeFalse()
        ->and(specSymbolResolves('LSNepomuceno\Signet\Signing\IncrementalSigner::methodThatWent'))->toBeFalse();
});

it('reads a symbol out of prose the way a comment writes it', function () {
    $cited = specSymbolReferences(
        'See \LSNepomuceno\Signet\Support\ProcessRunner and '
        . 'LSNepomuceno\Signet\Support\Bytes::overwrite() for the rest.',
        'somewhere',
    );

    expect(array_keys($cited))->toBe([
        'LSNepomuceno\Signet\Support\ProcessRunner',
        'LSNepomuceno\Signet\Support\Bytes::overwrite',
    ]);
});
