...
it('refuses a token that stamps something else, offline', function () {
    ...
    $signature = resolve(SignatureValidator::class)->validate($signed->contents)->latest();
    $cms = $signature->rawContents ?? '';
    $token = new LSNepomuceno\Signet\Validation\TimestampTokenReader()->read($cms);
