...
use LSNepomuceno\Signet\Tests\Harness;
use LSNepomuceno\Signet\Tests\TestCase;
...
|--------------------------------------------------------------------------
|
| There is no framework here and no application to boot. tests/Harness.php
| provides the three things the container was giving the suite: autowiring,
| rebinding and mutable configuration.
|
| Resetting before each test is not optional. A bind() or a setConfig() in one
| test would otherwise leak into every test that runs after it in the same
| process, which under --parallel makes a failure depend on scheduling.
|
*/
...
/**
 * Builds a collaborator, with its dependencies.
 *
...
{
    return Harness::instance()->make($class);
}
...
/**
 * Overrides one configuration value for the current test.
 *
 * The dot paths are the ones the Laravel package's config file uses, so a
 * behaviour described in one repository stays searchable in the other.
 */
function setConfig(string $key, mixed $value): void
{
    Harness::instance()->setConfig($key, $value);
}

/**
 * The package entry point, wired from the harness.
 *
 * Built fresh on every call rather than memoised: a test that changes
 * configuration or rebinds the transport expects the next resolution to see it.
 */
function signet(): LSNepomuceno\Signet\Signet
{
...
function deleteFiles(string ...$paths): void
{
    foreach ($paths as $path) {
        LSNepomuceno\Signet\Support\Files::delete($path);
    }
}

/**
 * A scratch path for a throwaway file.
 */
function tempFile(string $extension): string
{
    return new TempDirectory()->file($extension);
}
...
function pemCertificate(bool $encryptKey = true): array
{
    ...
    $privateKeyPath = tempFile('.key');
    $bundlePath = tempFile('.pem');
...
function testCertificate(): LSNepomuceno\Signet\Data\Certificate
{
    ...
    return resolve(LSNepomuceno\Signet\Certificates\NativeCertificateReader::class)
        ->read($pfx, $password);
}
...
{
    return LSNepomuceno\Signet\Support\Files::read(resource('signature-fields.pdf'));
}
