...
it('never makes a document structurally worse than it found it', function (string $case, string $source) {
    ...
    $signed = match ($case) {
        'plain' => signet()->newSignature()->certificate($pfxPath, $password)
            ->pdf(resource('test.pdf'))->sign(),

        'sealed' => signet()->newSignature()->certificate($pfxPath, $password)
        ...
        'xref-stream' => signet()->newSignature()->certificate($pfxPath, $password)
            ->pdf(resource('xref-stream.pdf'))->sign(),

        // Objects packed into an object stream, written back at the top level.
        'object-stream' => signet()->newSignature()->certificate($pfxPath, $password)
            ->pdf(resource('object-stream.pdf'))->sign(),

        // A predictor-compressed cross-reference stream.
        'predictor' => signet()->newSignature()->certificate($pfxPath, $password)
        ...
        'into-field' => signet()->newSignature()->certificate($pfxPath, $password)
            ->pdfContents(template(), 'contract.pdf')
            ->intoField('SignatureManager')->seal()->sign(),

        'certified' => signet()->newSignature()->certificate($pfxPath, $password)
            ->pdf(resource('test.pdf'))->certify()->lock()->sign(),

        'legacy' => signet()->newSignature()->certificate($pfxPath, $password)
...
it('keeps every revision sound as they stack up', function () {
    ...
    $pdf = LSNepomuceno\Signet\Support\Files::read(resource('test.pdf'));

    for ($round = 1; $round <= 4; $round++) {
        $pdf = signet()->newSignature()
