...
it('answers unknown rather than untrusted when no store was given', function () {
    ...
    $report = resolve(SignatureValidator::class)->validate(signedWithTestCertificate());
...
it('does not trust a signature against an unrelated root', function () {
    ...
    $other = resolve(\LSNepomuceno\Signet\Certificates\NativeCertificateReader::class)
...
it('reads a bundle from a file and from a directory', function () {
    ...
    $directory = new LSNepomuceno\Signet\Support\TempDirectory()->path() . '-trust';
...
it('reports trust through the entry point', function () {
    ...
    expect(signet()->validate($path, TrustStore::fromPem($root))->isTrusted())->toBeTrue()
        // And without a store, the same document answers unknown.
        ->and(signet()->validate($path)->isTrusted())->toBeNull();
