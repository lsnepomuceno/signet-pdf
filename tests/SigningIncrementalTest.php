...
use LSNepomuceno\Signet\Signing\Incremental\XrefSubsections;
...
it('leaves the original bytes untouched', function () {
    ...
    $signed = resolve(IncrementalSigner::class)
...
it('signs the same document three times without invalidating earlier signatures', function () {
    $signer = resolve(IncrementalSigner::class);
...
it('writes the signature metadata into the document', function () {
    ...
    $signed = resolve(IncrementalSigner::class)->sign(
        $contents,
        testCertificate(),
...
it('rejects a file that is not a PDF', function () {
    ...
    resolve(IncrementalSigner::class)->sign($contents, testCertificate(), new SignatureInfo());
...
it('reads the cross-reference chain of an unsigned document', function () {
    $document = resolve(DocumentReader::class)->read(Files::read(resource('test.pdf')));
...
it('lets the caller choose the transport after signing', function () {
    ...
    // download() and toResponse() are gone: a signing core does not return
    // HTTP responses (docs/decisions/0100-the-core-is-framework-agnostic.md).
    // What replaced them is a destination, and the Laravel package adds the
    // two response helpers back where a response is the natural currency.
    expect($signed->contents())->toBe((string) $signed)
        ->and(Files::read($path))->toBe($signed->contents)
        ->and($signed->size())->toBe(strlen($signed->contents))
        ->and($signed->name())->toEndWith('.pdf');
...
it('reads a cross-reference stream', function () {
    ...
    $document = resolve(DocumentReader::class)->read(Files::read(resource('xref-stream.pdf')));
...
it('refuses an encrypted document rather than corrupting it', function () {
    ...
    expect(fn() => resolve(DocumentReader::class)->read(Files::read(resource('encrypted.pdf'))))
...
it('names the structural fault instead of blaming the file extension', function () {
    ...
    try {
        resolve(DocumentReader::class)->read('not a pdf at all');
...
it('signs a cross-reference stream document by appending another stream', function () {
    ...
    $document = resolve(DocumentReader::class)->read($signed->contents);
