...
- **A Packagist outage no longer blocks an unrelated pull request.**
  `composer audit` failed on three unrelated branches on 2026-09-02, one of them
  the release, every time because the advisory database answered 502 and every
  time cleared by a rerun that changed nothing. The step now tries three times,
  and only on the message that says the database was unreachable: an advisory is
  still reported on the first attempt, and a sustained outage is still red. It is
  deliberately not `--ignore-unreachable`, which would turn an outage into an
  audit that silently did not happen
  ([0134](docs/decisions/0134-the-audit-retries-a-transient-outage.md)).
...
[Unreleased]: https://github.com/lsnepomuceno/signet-pdf/compare/3.0.0...HEAD
