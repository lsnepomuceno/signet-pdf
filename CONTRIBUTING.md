...
We accept contributions via Pull Requests on [Github](https://github.com/lsnepomuceno/signet-pdf).
...
- **Nothing is pushed to `main`.** Every change arrives through a pull request, without
  ...
  [#238](https://github.com/lsnepomuceno/signet-pdf/pull/238) and
  [#239](https://github.com/lsnepomuceno/signet-pdf/pull/239). Pushing a release
...
- **Check for a Symfony component before writing a helper** - and write your own only after
  establishing that none fits, saying in the docblock what was missing. **Any vendor that is not
  Symfony is a conversation to have before the code is written**, including development
  dependencies. The one standing exception is byte work on PDF and DER, where the multibyte
  helpers return wrong offsets: see [the conventions](docs/spec/conventions.md).

- **Nothing in `src/` may import a framework** - not `Illuminate\`, not `Laravel\`, not
  `Orchestra\`. This is the rule the package rests on and `tests/Project/ArchTest.php` fails on it
  (invariant 11).
...
The package targets PHP 8.4 and 8.5. Your machine almost certainly runs an
older PHP, so `.docker/` is not a convenience here but the normal way to run
anything: it also carries veraPDF, pyHanko, qpdf and `pdfsig`, which several
gates depend on.
